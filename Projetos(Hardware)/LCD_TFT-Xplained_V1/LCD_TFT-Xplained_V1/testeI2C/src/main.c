...
void Adafruit_FT6206writeRegister8(uint8_t reg, uint8_t val) {
	// use i2c
	/*
	i2cbeginTransmission(FT62XX_ADDR);
	i2cwrite((byte)reg);
	i2cwrite((byte)val);
	i2cendTransmission();
	*/
...
uint8_t Adafruit_FT6206readRegister8(uint8_t reg_addr) {
	
	

	//i2c_bus_write(uint8_t FT62XX_ADDR, uint8_t reg_addr, uint8_t *reg_data, uint8_t cnt)
//twihs_master_write(TWIHS_MCU6050, &p_packet);
//twihs_master_read(TWIHS_MCU6050, &p_packet);
...
int main(void){
	...
	i2c_bus_read(FT62XX_ADDR, FT62XX_REG_VENDID, &rtn8, 1);
	/*
	twihs_master_write(TWIHS_MCU6050, FT62XX_ADDR);
	twihs_write_byte(TWIHS_MCU6050, FT62XX_REG_VENDID);
	twihs_master_read(TWIHS_MCU6050, FT62XX_ADDR);
	rtn = twihs_read_byte(TWIHS_MCU6050);
	*/
  ...
  //
	...
	//}
	//else{
		//printf("[OK] [mcu] [Right Device [0x%2X] \n", bufferRX[0]);
	//}
	//delay_ms(2000);
	//
	//// Set Clock source
	//bufferTX[0] = MPU6050_CLOCK_PLL_XGYRO;
	//rtn = i2c_bus_write(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_PWR_MGMT_1, bufferTX, 1);
	...
	while (1) {
		...
		/*
		i2c_bus_read(FT62XX_ADDR, FT62XX_NUM_XL,&posXL, 1);
		i2c_bus_read(FT62XX_ADDR, FT62XX_NUM_XL,&posXL, 1);
		printf("X = %d, ", posXL);
		
		
		
		i2c_bus_read(FT62XX_ADDR, FT62XX_NUM_YH,&posYH, 1);
		i2c_bus_read(FT62XX_ADDR, FT62XX_NUM_YH,&posYH, 1);
		posYH = posYH & 0b0000111;
		i2c_bus_read(FT62XX_ADDR, FT62XX_NUM_YL,&posYL, 1);
		i2c_bus_read(FT62XX_ADDR, FT62XX_NUM_YL,&posYL, 1);
		posY = (posYH << 8) + posYL;
		printf("Y = %d\n", posY);
		...
		*/
		...
		//bufferTX[0] = 0x00; // 2G
		//rtn = i2c_bus_write(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ACCEL_CONFIG, bufferTX, 1);
		  //
		//// Le valor do acc X High e Low
		//rtn = i2c_bus_read(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ACCEL_XOUT_H, &raw_acc_xHigh, 1);
		//rtn = i2c_bus_read(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ACCEL_XOUT_L, &raw_acc_xLow,  1);
   //
		//// Le valor do acc y High e  Low
		//rtn = i2c_bus_read(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ACCEL_YOUT_H, &raw_acc_yHigh, 1);
		//rtn = i2c_bus_read(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ACCEL_ZOUT_L, &raw_acc_yLow,  1);
    //
		//// Le valor do acc z HIGH e Low
		//rtn = i2c_bus_read(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ACCEL_ZOUT_H, &raw_acc_zHigh, 1);
		//rtn = i2c_bus_read(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ACCEL_ZOUT_L, &raw_acc_zLow,  1);
     //
		...
		//rtn = i2c_bus_write(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_GYRO_CONFIG, bufferTX, 1);
				//
		//// Le valor do gyr X High e Low
		//rtn = i2c_bus_read(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_GYRO_XOUT_H, &raw_gyr_xHigh, 1);
		//rtn = i2c_bus_read(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_GYRO_XOUT_L, &raw_gyr_xLow,  1);
				//
		//// Le valor do gyr y High e  Low
		//rtn = i2c_bus_read(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_GYRO_YOUT_H, &raw_gyr_yHigh, 1);
		//rtn = i2c_bus_read(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_GYRO_ZOUT_L, &raw_gyr_yLow,  1);
				//
		//// Le valor do gyr z HIGH e Low
		//rtn = i2c_bus_read(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_GYRO_ZOUT_H, &raw_gyr_zHigh, 1);
		//rtn = i2c_bus_read(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_GYRO_ZOUT_L, &raw_gyr_zLow,  1);
				//
		//// Dados são do tipo complemento de dois
		...
		//printf("proc_gyr_x: %f, proc_gyr_y: %f, proc_gyr_z: %f\n", proc_gyr_x, proc_gyr_y, proc_gyr_z);
		//delay_ms(1000);
